...
get_day_length_data <- function(latitudes, longitudes, year) {
  day_length <- function(lat, lon, month) {
    date_range <- seq.Date(as.Date(paste0(year, "-", sprintf("%02d", month), "-01")),
                           as.Date(paste0(year, "-", sprintf("%02d", month), "-01")) + days_in_month(as.Date(paste0(year, "-", sprintf("%02d", month), "-01"))),
                           by = "day")
    lengths <- sapply(date_range, function(date) {
      times <- getSunlightTimes(date, lat = lat, lon = lon)
  ...
  day_length_data <- data.frame()
  for (i in seq_along(latitudes)) {
    lat <- latitudes[i]
    lon <- longitudes[i]
    for (month in 1:12) {
      length <- day_length(lat, lon, month)
      hours <- floor(length)
      minutes <- round((length - hours) * 60)
      day_length_data <- rbind(day_length_data, data.frame(
        Latitude = lat,
        Longitude = lon,
        Year = year,
        Month = month,
        DayLength = length,
        DayLengthHHMM = sprintf("%02d:%02d", hours, minutes)  # New column with HH:MM format
      ))
    }
  }
...
server <- function(input, output) {
    ...
    day_length_data <- eventReactive(input$update, {
      get_day_length_data(latitudes(), longitudes(), year = input$years[1])  # Only using the first year for day length calculation
    })
    ...
    day_length_plot <- reactive({
      ggplot(day_length_data(), aes(x = Month, y = DayLength, color = interaction(Latitude, Longitude))) +
        geom_line() +
        ...
        theme(legend.text.align = 1,
              legend.title = element_text(hjust = 1))  # Align legend to the right
    })
    ...
    output$dayLengthTable <- renderDT({
      datatable(
        day_length_data()[, c("Latitude", "Longitude", "Month", "DayLengthHHMM")],  # Include the new column
